...
while 1:
    ...
    if state == '1' or state == '2' or state == '3':
        if state == '1':
            update = models.execute_kw(db, uid, password, 'product.template', 'write', [[int(document['id'])], {'type': 'consu'}])
            print("Cambio realzado")
            break
        if state == '2':
            update = models.execute_kw(db, uid, password, 'product.template', 'write', [[int(document['id'])], {'type': 'service'}])
            print("Cambio realzado")
            break
        if state == '3':
            update = models.execute_kw(db, uid, password, 'product.template', 'write', [[int(document['id'])], {'type': 'product'}])
            print("Cambio realzado")
            break
    else:
